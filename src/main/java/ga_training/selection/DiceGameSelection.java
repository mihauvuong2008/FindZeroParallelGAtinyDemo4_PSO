...
//		System.out.println("sigmaReduce " + sigmaReduce + ", averageCount: " + averageCount + ", average " + average);
	...
	private final GENE randomDiceSearch(double[] remakeCandidateSet, ArrayList<EvaluatedCandidate> candidateSet,
			Random dice1, Random dice2, Random dice3, Random ran, double average, double sigmaReduce) {
		...
		while (candidate == null) {
			...
			if (toss < len) {
				delta = sigmaReduce * ran.nextDouble() * (ran.nextDouble() > 0.5 ? 1d : -1d);
